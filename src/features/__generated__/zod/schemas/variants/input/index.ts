/**
 * Input Variant Schemas
 * Auto-generated - do not edit manually
 */

export { AcquisitionInputSchema } from './Acquisition.input';
export { AcquisitionItemInputSchema } from './AcquisitionItem.input';
export { AcquisitionSpecJobInputSchema } from './AcquisitionSpecJob.input';
export { BankInputSchema } from './Bank.input';
export { BrandInputSchema } from './Brand.input';
export { ComplicationInputSchema } from './Complication.input';
export { CustomerInputSchema } from './Customer.input';
export { InvoiceInputSchema } from './Invoice.input';
export { InvoiceItemInputSchema } from './InvoiceItem.input';
export { MaintenancePartInputSchema } from './MaintenancePart.input';
export { MaintenanceRecordInputSchema } from './MaintenanceRecord.input';
export { MarketSegmentInputSchema } from './MarketSegment.input';
export { MechanicalPartCatalogInputSchema } from './MechanicalPartCatalog.input';
export { NotificationInputSchema } from './Notification.input';
export { NotificationRuleInputSchema } from './NotificationRule.input';
export { NotificationDispatchInputSchema } from './NotificationDispatch.input';
export { NotificationChannelDeliveryInputSchema } from './NotificationChannelDelivery.input';
export { NotificationRecipientGroupInputSchema } from './NotificationRecipientGroup.input';
export { BusinessEventLogInputSchema } from './BusinessEventLog.input';
export { ProjectionEventDeliveryInputSchema } from './ProjectionEventDelivery.input';
export { ProjectionRecordInputSchema } from './ProjectionRecord.input';
export { OrderInputSchema } from './Order.input';
export { PurchaseRequestInputSchema } from './PurchaseRequest.input';
export { PurchaseRequestIngressReceiptInputSchema } from './PurchaseRequestIngressReceipt.input';
export { PurchaseRequestItemInputSchema } from './PurchaseRequestItem.input';
export { PurchaseRequestActivityInputSchema } from './PurchaseRequestActivity.input';
export { OrderItemInputSchema } from './OrderItem.input';
export { IntegrationIngressReceiptInputSchema } from './IntegrationIngressReceipt.input';
export { PartVariantSpecInputSchema } from './PartVariantSpec.input';
export { PaymentInputSchema } from './Payment.input';
export { ProductInputSchema } from './Product.input';
export { PostTargetInputSchema } from './PostTarget.input';
export { ProductPostTargetInputSchema } from './ProductPostTarget.input';
export { ProductCategoryInputSchema } from './ProductCategory.input';
export { ProductContentInputSchema } from './ProductContent.input';
export { ProductImageInputSchema } from './ProductImage.input';
export { ProductVariantInputSchema } from './ProductVariant.input';
export { ReservationInputSchema } from './Reservation.input';
export { ServiceCatalogInputSchema } from './ServiceCatalog.input';
export { ServiceRequestInputSchema } from './ServiceRequest.input';
export { ShipmentInputSchema } from './Shipment.input';
export { ShipmentPackageInputSchema } from './ShipmentPackage.input';
export { CarrierRequestInputSchema } from './CarrierRequest.input';
export { CarrierWebhookDeliveryInputSchema } from './CarrierWebhookDelivery.input';
export { CarrierStatusHistoryInputSchema } from './CarrierStatusHistory.input';
export { CarrierChargeInputSchema } from './CarrierCharge.input';
export { StrapVariantSpecInputSchema } from './StrapVariantSpec.input';
export { ClaspVariantSpecInputSchema } from './ClaspVariantSpec.input';
export { WatchStrapInstallationInputSchema } from './WatchStrapInstallation.input';
export { StrapInventoryMovementInputSchema } from './StrapInventoryMovement.input';
export { StrapCatalogOptionInputSchema } from './StrapCatalogOption.input';
export { SupplyCatalogInputSchema } from './SupplyCatalog.input';
export { TechnicalAssessmentInputSchema } from './TechnicalAssessment.input';
export { TechnicalIssueInputSchema } from './TechnicalIssue.input';
export { TechnicalDetailCatalogInputSchema } from './TechnicalDetailCatalog.input';
export { UserInputSchema } from './User.input';
export { RoleInputSchema } from './Role.input';
export { PermissionInputSchema } from './Permission.input';
export { VendorInputSchema } from './Vendor.input';
export { WatchInputSchema } from './Watch.input';
export { WatchContentInputSchema } from './WatchContent.input';
export { WatchReviewStateInputSchema } from './WatchReviewState.input';
export { WatchReviewLogInputSchema } from './WatchReviewLog.input';
export { WatchPriceInputSchema } from './WatchPrice.input';
export { WatchSpecInputSchema } from './WatchSpec.input';
export { WatchSpecV2InputSchema } from './WatchSpecV2.input';
export { approvalRequestsInputSchema } from './approvalRequests.input';
export { SystemJobControlInputSchema } from './SystemJobControl.input';
export { SystemJobRunLogInputSchema } from './SystemJobRunLog.input';
export { technicalActionCatalogInputSchema } from './technicalActionCatalog.input';
export { technicalAppearanceIssueCatalogInputSchema } from './technicalAppearanceIssueCatalog.input';
export { technicalAssessmentsInputSchema } from './technicalAssessments.input';
export { technicalPartCatalogInputSchema } from './technicalPartCatalog.input';
export { AcquisitionSpecJobLogInputSchema } from './AcquisitionSpecJobLog.input';
export { MediaAssetInputSchema } from './MediaAsset.input';
export { MediaObjectInputSchema } from './MediaObject.input';
export { MediaBindingInputSchema } from './MediaBinding.input';
export { MediaOperationInputSchema } from './MediaOperation.input';
export { MediaLegacyManifestInputSchema } from './MediaLegacyManifest.input';
export { WorkCaseCategoryInputSchema } from './WorkCaseCategory.input';
export { WorkCaseInputSchema } from './WorkCase.input';
export { WorkCaseActivityInputSchema } from './WorkCaseActivity.input';
export { WorkflowTemplateInputSchema } from './WorkflowTemplate.input';
export { WorkflowConditionInputSchema } from './WorkflowCondition.input';
export { WorkflowActionInputSchema } from './WorkflowAction.input';
export { WorkflowExecutionInputSchema } from './WorkflowExecution.input';
export { WorkflowExecutionEventInputSchema } from './WorkflowExecutionEvent.input';
export { BusinessFeedbackInputSchema } from './BusinessFeedback.input';
export { TimelineEntryInputSchema } from './TimelineEntry.input';
export { UserCommentInputSchema } from './UserComment.input';
export { TaskInputSchema } from './Task.input';
export { TaskItemInputSchema } from './TaskItem.input';
export { TaskItemActivityInputSchema } from './TaskItemActivity.input';
export { TaskItemActivityReplyInputSchema } from './TaskItemActivityReply.input';
export { TaskItemChecklistInputSchema } from './TaskItemChecklist.input';
export { TaskExecutionInputSchema } from './TaskExecution.input';
export { AppTagInputSchema } from './AppTag.input';
export { AppTagLinkInputSchema } from './AppTagLink.input';
