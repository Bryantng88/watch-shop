...
function buildInitialFilters(
  rawSearchParams: ShipmentListPageProps["rawSearchParams"],
  pageSize: number,
): ShipmentListFiltersValue {
  return {
...
export default function ShipmentListClient({
  ...
  const [returnFeeShipment, setReturnFeeShipment] =
  ...
  const currentView = normalizeShipmentView(
    sp.get("view") || (rawSearchParams.view as any),
  );
  const [filters, setFilters] = useState<ShipmentListFiltersValue>(() =>
  ...
  function navigateWithLoading(patch: Record<string, string | null | undefined>) {
    progress.show({
      title: "Đang tải shipment",
      message: "Hệ thống đang cập nhật danh sách giao hàng.",
    });
  ...
  async function mutate(input: {
    ...
    progress.show({
      ...
      message: "Hệ thống đang xử lý shipment.",
    });
  ...
  async function submitFee(payload: unknown) {
    ...
    await mutate({
      ...
      successMessage:
        "Thông tin vận chuyển đã được cập nhật và shipment đã chuyển sang Đang giao.",
  ...
  async function submitReturnFee(payload: unknown) {
    ...
    await mutate({
      url: `/api/admin/shipments/${returnFeeShipment.id}/receive-return`,
      body: payload,
      progressTitle: "Đang nhận hàng hoàn",
      successTitle: "Đã nhận hàng hoàn",
      successMessage:
        "Shipment đã kết thúc vòng đời hoàn trả. Nếu có phí hoàn, payment OUT đã được ghi nhận.",
  ...
  async function markDelivered(row: ShipmentListItem) {
    const ok = await dialog.confirm({
      tone: "success",
      title: isCodShipment(row)
        ? "Xác nhận đã giao shipment COD?"
  ...
  async function markReturned(row: ShipmentListItem) {
    const ok = await dialog.confirm({
      tone: "danger",
      title: "Xác nhận shipment đang hoàn?",
      message:
        "Shipment sẽ chuyển sang trạng thái Đang hoàn. Order và watch vẫn giữ nguyên trạng thái để sale tiếp tục theo dõi cho tới khi nhận lại hàng.",
      confirmText: "Đang hoàn",
      cancelText: "Hủy",
    });
    ...
    await mutate({
      ...
      progressTitle: "Đang cập nhật hoàn trả",
      successTitle: "Đã chuyển sang Đang hoàn",
      successMessage:
        "Shipment đã được đánh dấu đang hoàn về shop. Khi nhận lại hàng, hãy dùng action Nhận hàng hoàn để nhập phí hoàn chính xác.",
      errorTitle: "Không thể cập nhật hoàn trả",
    });
  }
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <ShipmentListViewTabs value={currentView} onChange={setView} />
          </div>
