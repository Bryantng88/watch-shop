...
export default function ShipmentListClient({ items, total, page, pageSize, totalPages, rawSearchParams }: ShipmentListPageProps) {
  ...
  async function submitFee(payload: unknown) {
    ...
    await mutate({
      ...
      body: payload,
      progressTitle: "Đang tạo phí ship",
      successTitle: "Đã tạo phí ship",
      successMessage: "Payment phí ship đã hoàn tất và shipment đã chuyển sang Đang giao.",
      errorTitle: "Không thể tạo phí ship",
  ...
  async function markReturned(row: ShipmentListItem) {
    const ok = await dialog.confirm({
      tone: "danger",
      title: "Xác nhận hoàn trả shipment?",
      message: "Shipment sẽ chuyển sang Hoàn trả và order cũng cần được đưa về trạng thái xử lý phù hợp để sale theo dõi lại.",
    ...
    await mutate({
      ...
      progressTitle: "Đang hoàn trả shipment",
      successTitle: "Đã hoàn trả shipment",
      errorTitle: "Không thể hoàn trả shipment",
    });
  }
