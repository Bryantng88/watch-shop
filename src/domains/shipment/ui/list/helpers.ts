...
export function normalizeShipmentView(value?: string | null): ShipmentViewKey {
  ...
  if (["ready", "shipping", "delivered", "returned", "cancelled"].includes(raw)) return raw as ShipmentViewKey;
  return "all";
}
...
export function shipmentStatusTone(status?: string | null) {
  switch (String(status ?? "").toUpperCase()) {
    ...
    case "RETURNED": return "bg-orange-50 text-orange-700 ring-orange-200";
