...
import { ProductStatus, ServiceRequestStatus, ServiceScope, ServiceType } from "@prisma/client";
...
export async function createTechnicalChecksFromProductsApplication(input: CreateTechnicalChecksFromProductsInput) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const productId of ids) {
      ...
      const refNo = await genRefNo(tx as any, { model: (tx as any).serviceRequest, prefix: "SR", field: "refNo", padding: 6 });
      ...
      const request = await repo.createTechnicalCheckRequest(tx, {
        ...
        technicianNameSnap: technician?.name ?? technician?.email ?? null,
      });
