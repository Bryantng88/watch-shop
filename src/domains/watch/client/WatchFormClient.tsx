  ...
  return (
    <button
      ...
      title={
          ...
          ? "Lưu cập nhật và giữ trạng thái đã hoàn tất"
          : "Lưu và xác nhận mục này đã xong"
      }
    >
...
function mediaApproveNotificationStep(
  ...
  const sentItem = items.find(
    (item) => item.ok === true && item.skipped !== true && item.dispatchId,
  );
...
export default function WatchFormClient({
  ...
  const setFormValues = (
    updater: (prev: WatchFormValues) => WatchFormValues,
  ) => {
    setValues((prev) => {
      ...
      .catch((error: unknown) => {
        ...
        notify.error({
          ...
          message: errorMessage(
            error,
            "Kho ảnh chưa tải được. Bạn có thể thử làm mới modal.",
          ),
        });
      });
  ...
  const saveMediaWorkspacePart = async (part: MediaWorkPart) => {
    ...
    try {
      ...
      if (progressResult.skipped || !("parts" in progressResult)) {
        throw new Error(
          "Không tìm thấy item Xử lý Media đang mở để lưu tiến độ.",
        );
      }
      ...
      try {
        ...
        if (workspaceResult?.skipped) {
          ...
          notify.success({
            ...
            message:
              "Ảnh đã được lưu và item đã chuyển sang trạng thái chờ duyệt trong Workspace.",
          });
        }
  ...
  const requestMediaReshootFromModal = async () => {
    ...
    let steps: AppProgressStep[] = [
      ...
      },
      {
        ...
        detail:
          "Áp dụng action request-changes qua Workspace Workflow Processor.",
        status: "pending",
      },
      {
        id: "reconcile",
    ...
    try {
      ...
      if (!workflowResult?.applied || workflowResult.toState !== "RETURNED") {
        throw new Error(
      ...
      steps = setStepStatus(
        ...
        "transition",
        "done",
        "Workflow đã chuyển khỏi Media Processing.",
      );
      steps = setStepStatus(
        steps,
        "reconcile",
        "running",
        "Đang chờ Photography và các bộ đếm đồng bộ.",
      );
      progress.update({ steps });

      const reshootEffect =
        transitionResult.workflowProcessorResult.effects.find(
      ...
      steps = setStepStatus(
        ...
        "reconcile",
        "done",
        "Danh sách cha đã nhận kết quả chuyển stage.",
      );
      progress.update({
  ...
  const approveMediaWorkspaceFromModal = async () => {
    ...
    if (isMediaWorkspaceReturned) {
      await dialog.alert({
        ...
        message:
          "Can cap nhat anh moi tu Photoshoot truoc khi duyet qua Workspace Dang bai.",
        tone: "warning",
      });
      return;
    }
    ...
    let steps: AppProgressStep[] = [
      ...
      },
      {
        ...
        detail:
          "Gui Zalo notification theo rule watch.media.ready_for_publish.",
        status: "pending",
      },
      {
        id: "close",
    ...
    try {
      ...
      if (eventDeferred) {
        notify.success({
          ...
          message:
            "Media Ä‘Ã£ duyá»‡t; Publish workspace vÃ  notification Ä‘ang Ä‘Æ°á»£c Ä‘á»“ng bá»™ ná»n.",
        });
      } else if (notificationStep.status === "done") {
        notify.success({
          title: "Đã duyệt media",
          message:
            "Item đã được duyệt, chuyển sang Workspace Đăng bài và đã gửi notification.",
        });
      } else {
        notify.warning({
          title: "Đã duyệt media, notification chưa gửi",
          message:
            notificationStep.detail ?? "Notification chưa tạo dispatch thật.",
          duration: 6000,
        });
      }
      ...
      steps = setStepStatus(
        ...
        "close",
        "done",
        "Workspace da duoc lam moi.",
      );
      progress.update({
        message: eventDeferred
  ...
  const reopenMediaWorkspaceFromModal = async () => {
    ...
    try {
      ...
      notify.success({
        ...
        message:
          "Báº¡n cÃ³ thá»ƒ cáº­p nháº­t media vÃ  duyá»‡t láº¡i item nÃ y.",
      });
      router.refresh();
    } catch (error: unknown) {
      notify.error({
  ...
  const saveBeforeReview = async (target?: "content" | "image") => {
    ...
    try {
      ...
      notify.success({
        title: "Đã lưu watch",
        message:
          result?.message || "Dữ liệu mới nhất đã được lưu trước khi duyệt.",
      });
  ...
  const mediaDoneAction = (part: MediaWorkPart) =>
    isMediaProgressContext ? (
      <MediaWorkDoneButton
        ...
        disabled={
          mediaSubmitPending || isMediaWorkspaceDone || isMediaWorkspaceReturned
        }
            ...
            <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Ghi rõ phần Photoshoot cần chụp lại.
                </p>
              </div>
              <button
                type="button"
      ...
      {isMediaWorkspaceDone ? (
        ...
        <>
          ...
          <button
            ...
            title={
              isMediaWorkspaceReturned
                ? "Item dang o trang thai Returned, can xu ly lai anh truoc khi duyet."
                : undefined
            }
