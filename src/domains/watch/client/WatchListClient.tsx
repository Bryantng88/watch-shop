...
function sanitizeParams(input: URLSearchParams) {
  ...
  ["subFilter", "hasContent", "saleStage", "opsStage", "quickFilter"].forEach(
  ...
  params.set(
    "page",
    Number.isFinite(page) && page > 0 ? String(Math.floor(page)) : "1",
  );
...
export default function WatchListClient(props: WatchListClientProps) {
  ...
  const [serviceIntakeProductId, setServiceIntakeProductId] = React.useState<
    string | null
  >(null);
  const [serviceIntakeRow, setServiceIntakeRow] =
    React.useState<WatchRow | null>(null);
  const [serviceSuspicion, setServiceSuspicion] = React.useState("");
  const [serviceIntakeError, setServiceIntakeError] = React.useState<
    string | null
  >(null);
  ...
  const urlParams = useMemo(
    () => sanitizeParams(new URLSearchParams(sp.toString())),
    [sp],
  );
  ...
  const [filters, setFilters] = useState<WatchFilterFormState>(() =>
    filterStateFromParams(urlParams),
  );
  ...
  const [mediaRowSubmittingId, setMediaRowSubmittingId] = useState<
    string | null
  >(null);
  const [duplicateSubmittingId, setDuplicateSubmittingId] = useState<
    string | null
  >(null);
  ...
  const photoshootEligibleRows = useMemo(
    () =>
      selectedRows.filter(
        (row) => !row.hasImages && Number(row.imagesCount ?? 0) <= 0,
      ),
    [selectedRows],
  );
  ...
  function handleFilterChange(patch: Partial<WatchFilterFormState>) {
    setFilters((prev) => {
      ...
      if (
        Object.prototype.hasOwnProperty.call(patch, "priceStatus") &&
        patch.priceStatus
      ) {
        next.pricePreset = "";
        next.priceMin = "";
      ...
      if (
        Object.prototype.hasOwnProperty.call(patch, "pricePreset") &&
        patch.pricePreset
      ) {
        next.priceStatus = "";
        next.priceMin = "";
      ...
      if (
        (Object.prototype.hasOwnProperty.call(patch, "priceMin") &&
          patch.priceMin) ||
        (Object.prototype.hasOwnProperty.call(patch, "priceMax") &&
          patch.priceMax)
      ) {
        next.priceStatus = "";
        next.pricePreset = "";
  ...
  async function requestSelectedPhotoshoot() {
    ...
    const steps: AppProgressStep[] = photoshootEligibleRows.map(
      (row, index) => ({
        id: row.id,
    ...
    try {
      ...
      const itemByWatchId = new Map(
        result.items.map((item) => [item.watchId, item]),
      );
      const sentCount = result.requested ?? 0;
      const skippedCount = result.skipped ?? 0;

      progress.update({
        message: `Đang đồng bộ ${sentCount} watch vào danh sách...`,
        steps: steps.map((step) => {
          ...
          return {
            ...step,
            status: requested ? "running" : "skipped",
            detail: requested
              ? "Đã nhận lệnh, đang chờ projection..."
              : (item?.reason ?? "Watch được bỏ qua."),
          };
        }),
      });

      await waitForBulkProjectionDeliveries(
        result.items.filter((item) => item.status === "REQUESTED"),
        { concurrency: 4 },
      );

      progress.update({
        message: `${sentCount}/${total} watch đã đồng bộ`,
        steps: steps.map((step) => {
          const item = itemByWatchId.get(step.id);
          return item?.status === "REQUESTED"
            ? {
                ...step,
                status: "done",
                detail: "Đã đồng bộ vào WP Photoshoot.",
              }
            : {
                ...step,
                status: "skipped",
                detail: item?.reason ?? "Watch được bỏ qua.",
              };
        }),
      });
      ...
      progress.update({
        ...
        steps: steps.map((step) => ({
          ...
          status: "error",
          detail:
            error instanceof Error ? error.message : "Không thể gửi watch này.",
        })),
      });
      notify.error({
  ...
  async function requestSelectedMediaReview() {
    ...
    const steps: AppProgressStep[] = mediaReviewEligibleRows.map(
      (row, index) => ({
        id: row.id,
        label: row.title || row.sku || row.productId,
  ...
  async function sendWatchToMediaSpace(row: WatchRow) {
    ...
    try {
      if (row.hasImages) {
        ...
        await waitForBulkProjectionDeliveries([item]);
      }
      ...
      notify.error({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Có lỗi khi cập nhật Media Workspace.",
      });
    } finally {
  ...
  async function confirmDuplicateWatch(row: WatchRow) {
    ...
    try {
      ...
      notify.success({
        ...
        message:
          "Watch đã được đưa ra khỏi danh sách hiện tại. Dữ liệu vẫn được giữ nguyên.",
      });
      ...
      notify.error({
        ...
        message:
          error instanceof Error ? error.message : "Có lỗi khi cập nhật watch.",
      });
    } finally {
  ...
  async function sendSelectedToMediaSpace() {
    if (!selectedRows.length || photoshootSubmitting || mediaReviewSubmitting)
      return;
    ...
    const steps: AppProgressStep[] = selectedRows.map((row) => ({
      id: row.id,
      label: row.title || row.sku || row.productId,
      detail: row.hasImages
        ? "Chờ đưa vào Xử lý Media"
        : "Chờ đưa vào Chụp ảnh",
      status: "pending",
    }));
      ...
      try {
        if (row.hasImages) {
          ...
          if (result.skipped) {
            ...
            steps[index] = {
              ...steps[index],
              status: "done",
              detail: "Đã đưa vào Workspace Xử lý Media.",
            };
          }
        } else {
          const result = await requestWatchPhotoshootAction({
          ...
          if (item?.status !== "REQUESTED") {
            ...
            steps[index] = {
              ...steps[index],
              status: "skipped",
              detail: item?.reason ?? "Watch đã được bỏ qua.",
            };
          } else {
            await waitForBulkProjectionDeliveries([item], {
              cancelled: () => cancelled,
            ...
            steps[index] = {
              ...steps[index],
              status: "done",
              detail: "Đã đưa vào Workspace Chụp ảnh.",
            };
          }
        }
      } catch (error) {
        ...
        steps[index] = {
          ...
          detail:
            error instanceof Error
              ? error.message
              : "Không thể cập nhật Space Media.",
        };
      }
    ...
    try {
      if (cancelled) {
        notify.info({
          title: "Đã dừng batch",
          message:
            "Không nhận thêm item mới. Item đang gửi trước lúc dừng có thể vẫn hoàn tất ở server.",
        });
        await loadList(new URLSearchParams(params.toString()), {
          meta: "lite",
        });
  ...
  async function submitServiceIntake(openExisting = false) {
    ...
    try {
      ...
      intakeSteps = intakeSteps.map((step) =>
        step.id === "create"
          ? {
              ...step,
              status: "done",
              detail: "Phiếu kỹ thuật đã được ghi nhận.",
            }
          : step.id === "background"
            ? {
                ...step,
                status: "running",
                detail:
                  "Đã chuyển sang xử lý nền; bạn có thể tiếp tục làm việc.",
              }
            : step,
      );
      progress.update({
      ...
      notify.success({
        title: data.createdServiceRequest
          ? "Đã tạo phiếu kỹ thuật"
          : "Đã ghi nhận phiếu kỹ thuật",
  ...
  function onConsign(row: WatchRow) {
    navigateWithProgress(
      `/admin/consignments/new?productId=${row.productId}`,
      "Đang mở ký gửi",
    );
  }
  ...
  async function submitBuyBack(payload: {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Không thể tạo phiếu mua lại.";
    ...
    >
      {selectedRows.length && !duplicateView ? (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-violet-200 bg-violet-50/70 px-4 py-3 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {mediaReviewSubmitting
                ? "Đang xử lý..."
                : `Đưa vào Space Media (${selectedRows.length})`}
            </button>
            <button
              type="button"
      ...
      <div className={loading ? "opacity-60 transition" : "transition"}>
        <WatchListTable
          ...
          onRepairInline={
            WATCH_INLINE_REPAIR_ENABLED ? setInlineRepairRow : undefined
          }
        />
      </div>
