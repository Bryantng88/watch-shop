  ...
  const origin =
    t === "BRAND_ORIGINAL"
      ...
      : t === "COMPONENT"
        ...
        : "";
  if (!origin) return "";
  if (s === "FROM_STOCK") return `${origin}, lấy dây thay từ kho.`;
  if (s === "KEEP_CURRENT") return `${origin}, đi kèm đồng hồ.`;
  return `${origin}.`;
}
...
export function buildPostTitle(values: WatchFormValues) {
  ...
  return (
    [year, brand, model, reference, nickname, movement]
      .filter(Boolean)
      .join(" ") || baseTitle
  );
}
...
export function buildWatchContentWarnings(values: WatchFormValues) {
  ...
  if (!clean(values.spec.model)) {
    pushMissing(
      warnings,
      "model",
      "Chưa có model, bullet model sẽ được bỏ qua.",
    );
  }
  ...
  if (
    false &&
    clean(values.spec.strapSetType) &&
    !clean(values.spec.strapComponentSource)
  ) {
    pushMissing(
      warnings,
      "strapComponentSource",
      "Chưa chọn dây đi kèm đồng hồ hay lấy từ kho.",
    );
  }
