...
import { computeWatchSpecStatus } from "./acquisition-watch-spec-status.helper";
import { appendAcquisitionSpecJobLog } from "./acquisition-spec-job-log.service";
import { applyWatchTitleAndSkuForSpec } from "@/domains/watch/server/write/watch-write.service";
...
const ACQ_META_PREFIX = "__ACQ_META__:";

function unwrapAcquisitionMetaText(value?: string | null) {
    const text = String(value ?? "").trim();
    if (!text) return "";
    return text.startsWith(ACQ_META_PREFIX)
        ? text.slice(ACQ_META_PREFIX.length).trim()
        : text;
}

function safeJsonParse<T = JsonObject>(value?: string | null): T {
    const text = unwrapAcquisitionMetaText(value);
    if (!text) return {} as T;
    try {
...
function stringifyJson(value: unknown) {
    return `${ACQ_META_PREFIX}${JSON.stringify(value ?? {})}`;
}
...
async function markAcquisitionSpecJobRunning(input: {
    id: string;
}) {
    return prisma.$transaction(
        async (tx) => {
            return tx.acquisitionSpecJob.update({
                where: { id: input.id },
                data: {
                    status: "RUNNING",
                    startedAt: new Date(),
...
async function persistPreparedJobInTransaction(prepared: PreparedAcquisitionSpecJob) {
    return prisma.$transaction(
        async (tx) => {
...
async function failAcquisitionSpecJobById(input: {
    acquisitionItemId: string;
    errorMessage: string;
}) {
    return prisma.$transaction(
        async (tx) => {
            await failAcquisitionSpecJob(tx, input);
        },
        { timeout: 10000 }
    );
}
...
export async function processAcquisitionSpecJob(
    ...
    await prisma.acquisitionSpecJob.update({
        where: { id: job.id },
        data: {
            status: "RUNNING",
            startedAt: new Date(),
    ...
    await persistPreparedAcquisitionSpecJob(prisma as any, prepared);
}
...
async function markJobFailed(input: {
    ...
    await failAcquisitionSpecJob(prisma as any, input);
}
...
export async function processQueuedAcquisitionSpecJobs(input?: {
    ...
    for (const job of jobs) {
        try {
            ...
            // AI + S3 + mapping chạy ngoài transaction.
            const prepared = await prepareAcquisitionSpecJobData({
                jobId: job.id,
                acquisitionItemId: job.acquisitionItemId,
            });

            // Persist không bọc interactive transaction để tránh timeout.
            await persistPreparedAcquisitionSpecJob(prisma as any, prepared);
