...
export default function PublicOrderForm() {
  const { items, remove, clear } = useStorefrontCart();
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!items.length || state.kind === "submitting") return;
    if (!online) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
    <section><h2 className="storefront-display text-2xl">Sản phẩm</h2>{items.length ? <ul className="mt-5 divide-y divide-[#dedbd4] border-y border-[#dedbd4]">{items.map((item) => <li key={item.productId} className="flex items-center gap-4 py-4"><div className="relative h-20 w-16 shrink-0 overflow-hidden bg-[#efede8]"><Image src={item.imageUrl} alt="" fill sizes="64px" className="object-cover" /></div><div className="min-w-0 flex-1"><Link href={`/products/${item.slug}`} className="font-medium">{item.title}</Link><p className="mt-1 text-xs text-[#77746f]">Số lượng 1</p></div><button type="button" onClick={() => remove(item.productId)} className="storefront-focus grid h-11 w-11 place-items-center" aria-label={`Xóa ${item.title}`}><Trash2 className="h-4 w-4" /></button></li>)}</ul> : <p className="mt-5 text-[#77746f]">Chưa có sản phẩm. <Link className="underline" href="/products">Xem bộ sưu tập</Link>.</p>}</section>
    <form onSubmit={submit} className="space-y-5"><h2 className="storefront-display text-2xl">Thông tin liên hệ</h2><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" />
      ...
      <button disabled={!online || !items.length || state.kind === "submitting"} className="storefront-focus min-h-12 w-full bg-[#30302e] px-6 text-xs uppercase tracking-[0.15em] text-white disabled:cursor-not-allowed disabled:opacity-50">{state.kind === "submitting" ? "Đang gửi…" : online ? "Gửi yêu cầu" : "Offline — không thể gửi"}</button>
