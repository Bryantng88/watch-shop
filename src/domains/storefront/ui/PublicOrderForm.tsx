...
export default function PublicOrderForm({ initialItems = [], initialRequestKey, submittedReference, submittedDisposition, submittedAddedItemCount = 0, initialError }: { initialItems?: StorefrontCartItem[]; initialRequestKey: string; submittedReference?: string; submittedDisposition?: "CREATED" | "MERGED"; submittedAddedItemCount?: number; initialError?: string }) {
  ...
  if (state.kind === "success") return <div className="mx-auto max-w-xl py-20 text-center"><h1 className="storefront-display text-4xl">{state.disposition === "MERGED" ? "Đã bổ sung Watch" : "Đã nhận yêu cầu"}</h1><p className="mt-5 text-[#66635e]">{state.disposition === "MERGED" ? `Bạn vừa bổ sung thêm ${state.addedItemCount} Watch vào yêu cầu mua hàng.` : "Đội ngũ đã nhận yêu cầu mua hàng của bạn."}</p><p className="mt-2 font-medium text-[#3b3935]">Mã tham chiếu: {state.reference ?? "đang cập nhật"}</p><p className="mt-2 text-sm text-[#77746f]">Hãy cung cấp mã này khi cần trao đổi với đội ngũ tư vấn.</p><Link className="storefront-focus mt-8 inline-flex min-h-11 items-center border border-[#333] px-5 text-xs uppercase tracking-widest" href="/products">Tiếp tục xem</Link></div>;
  ...
  return <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
    ...
    <form action="/api/public/orders" method="post" onSubmit={submit} noValidate className="space-y-5"><h2 className="storefront-display text-2xl">Thông tin liên hệ</h2><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" />
      ...
      <button type="submit" disabled={!online || !effectiveItems.length || state.kind === "submitting"} className="storefront-focus min-h-12 w-full bg-[#30302e] px-6 text-xs uppercase tracking-[0.15em] text-white disabled:cursor-not-allowed disabled:opacity-50">{state.kind === "submitting" ? "Đang gửi…" : online ? "Gửi yêu cầu" : "Offline — không thể gửi"}</button>
