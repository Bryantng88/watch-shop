...
function resolvePaymentOwner(payment: any): {
    ...
    if (payment.technical_issue_id) return { ownerType: "SERVICE", ownerId: payment.technical_issue_id };
...
export async function ensureTechnicalIssuePaymentTx(tx: Tx, technicalIssueId: string) {
    ...
    if (seed.totalDue < 0) {
        throw new Error("Chi phí technical issue không hợp lệ.");
    }
...
export async function createPayment(input: CreatePaymentInput) {
    return prisma.$transaction(async (tx) => {
        ...
        const payment = await createPaymentRowTx(tx, {
            ...
            purpose:
                ...
                (input.ownerType === "ACQUISITION"
                    ? "ACQUISITION_FULL"
                    : input.ownerType === "SERVICE" || input.ownerType === "TECHNICAL_ISSUE"
        ...
        const serviceOwner = input.ownerType === "SERVICE" || input.ownerType === "TECHNICAL_ISSUE";
