...
export default function StandaloneExpensePaymentModal({
  ...
  const [kind, setKind] = useState<"SALARY" | "OPERATING_EXPENSE" | "OTHER_INCOME" | "OPENING_BALANCE">("OPERATING_EXPENSE");
  ...
  async function submit() {
    ...
    if ((markPaidNow || kind === "OPENING_BALANCE") && !effectiveAt) return setError("Vui lòng chọn ngày ghi nhận.");
    setSubmitting(true);
    try {
      const response = await fetch("/api/admin/payments/expenses", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ kind, amount: numericAmount, method, financeChannel, payeeUserId, expenseCategoryId, payeeName, reference, note, markPaidNow, effectiveAt }),
      });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || "Không thể tạo khoản thu/chi.");
      setAmount(""); setReference(""); setNote(""); setPayeeName(""); setMarkPaidNow(false);
      await onCreated();
      onClose();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Không thể tạo khoản thu/chi.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4">
    <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
        <div className="flex gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-950 text-white"><ReceiptText className="h-5 w-5" /></span><div><h2 className="font-bold text-slate-950">Tạo khoản thu / chi</h2><p className="mt-1 text-xs text-slate-500">Payment ngoài domain nghiệp vụ; vẫn phát event và cập nhật Finance Report.</p></div></div>
      ...
      <div className="space-y-4 p-6">
        <div className="grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1 sm:grid-cols-4">
          {([['OPERATING_EXPENSE','Chi phí'],['SALARY','Lương'],['OTHER_INCOME','Thu khác'],['OPENING_BALANCE','Số dư đầu']] as const).map(([value,label]) => <button key={value} type="button" onClick={() => setKind(value)} className={`h-10 rounded-lg text-sm font-semibold ${kind === value ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500'}`}>{label}</button>)}
        </div>
        {loading ? <div className="flex h-24 items-center justify-center text-sm text-slate-500"><LoaderCircle className="mr-2 h-4 w-4 animate-spin" />Đang tải...</div> : <>
          {kind === "SALARY" ? <label className="block text-xs font-semibold text-slate-600">Nhân sự nhận lương<select value={payeeUserId} onChange={(e) => setPayeeUserId(e.target.value)} className={`${fieldClass} mt-1.5`}><option value="">Chọn user</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name || user.email} {user.name && user.email ? `· ${user.email}` : ''}</option>)}</select></label> : null}
          {kind === "OPERATING_EXPENSE" ? <><label className="block text-xs font-semibold text-slate-600">Loại chi phí<select value={expenseCategoryId} onChange={(e) => setExpenseCategoryId(e.target.value)} className={`${fieldClass} mt-1.5`}><option value="">Chọn loại chi phí</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label className="block text-xs font-semibold text-slate-600">Người / đơn vị nhận (không bắt buộc)<input value={payeeName} onChange={(e) => setPayeeName(e.target.value)} className={`${fieldClass} mt-1.5`} placeholder="Ví dụ: Chủ nhà, Điện lực..." /></label></> : null}
          {kind === "OTHER_INCOME" ? <label className="block text-xs font-semibold text-slate-600">Nguồn thu / người nộp<input value={payeeName} onChange={(e) => setPayeeName(e.target.value)} className={`${fieldClass} mt-1.5`} placeholder="Ví dụ: Thanh lý tài sản, hoàn phí..." /></label> : null}
          {kind === "OPENING_BALANCE" ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800">Số dư đầu kỳ là mốc chuyển hệ thống. Khoản này chỉ thiết lập cash balance, không được tính là doanh thu hoặc lợi nhuận.</div> : null}
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-xs font-semibold text-slate-600">Ngày ghi nhận<input type="date" value={effectiveAt} onChange={(e) => setEffectiveAt(e.target.value)} disabled={!markPaidNow && kind !== "OPENING_BALANCE"} className={`${fieldClass} mt-1.5 disabled:bg-slate-50 disabled:text-slate-400`} /></label><label className="flex items-end gap-2 pb-3 text-sm text-slate-700"><input type="checkbox" checked={kind === "OPENING_BALANCE" || markPaidNow} disabled={kind === "OPENING_BALANCE"} onChange={(e) => setMarkPaidNow(e.target.checked)} />{kind === "OPENING_BALANCE" ? "Mốc số dư đã xác nhận" : kind === "OTHER_INCOME" ? "Đã thu tiền" : "Đã chi tiền"}</label></div>
        </>}
        {error ? <div className="rounded-xl bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">{error}</div> : null}
      </div>
      <div className="flex justify-end gap-2 border-t border-slate-100 px-6 py-4"><button type="button" onClick={onClose} className="h-10 rounded-xl border border-slate-200 px-4 text-sm font-semibold">Hủy</button><button type="button" disabled={loading || submitting} onClick={() => void submit()} className="h-10 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white disabled:opacity-50">{submitting ? "Đang tạo..." : "Tạo khoản thu / chi"}</button></div>
    </div>
  </div>;
}
