...
export async function listTechnicalDetailCatalog() {
    return prisma.technicalDetailCatalog.findMany({
        orderBy: [{ area: "asc" }, { sortOrder: "asc" }, { name: "asc" }],
    });
}

export async function upsertTechnicalDetailCatalog(input: any) {
    const data = {
        code: String(input.code || "").trim(),
        area: String(input.area || "GENERAL").trim(),
        name: String(input.name || "").trim(),
    ...
    if (!data.code || !data.name) {
        throw new Error("Thiếu code hoặc tên chi tiết kỹ thuật");
    }

    if (input.id) {
        return prisma.technicalDetailCatalog.update({
            where: { id: String(input.id) },
            data,
        });
    }

    return prisma.technicalDetailCatalog.upsert({
        where: { code: data.code },
        update: data,
        create: data,
    });
