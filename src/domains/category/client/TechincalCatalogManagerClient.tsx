...
function emptyTechnicalDetailForm() {
    return {
        id: "",
        code: "",
        area: "CASE",
...
export default function TechnicalCatalogManagerClient({
    ...
    const [technicalDetailForm, setTechnicalDetailForm] = React.useState(emptyTechnicalDetailForm());
    ...
    const [editingTechnicalDetailId, setEditingTechnicalDetailId] = React.useState<string | null>(null);
    ...
    async function handleSaveTechnicalDetail() {
        try {
            setSubmitting("technicalDetail");
            await save("technicalDetail", technicalDetailForm);
            setTechnicalDetailForm(emptyTechnicalDetailForm());
            setEditingTechnicalDetailId(null);
        } catch (error) {
            alert(error instanceof Error ? error.message : "Lưu chi tiết kỹ thuật thất bại");
        } finally {
            setSubmitting(null);
        }
    }
    ...
    function startEditTechnicalDetail(item: TechnicalDetailItem) {
        setEditingTechnicalDetailId(item.id);
        setTechnicalDetailForm({
            id: item.id,
            code: item.code,
            area: item.area,
            name: item.name,
    ...
    async function toggleTechnicalDetailActive(item: TechnicalDetailItem) {
        try {
            await save("technicalDetail", {
                ...item,
                isActive: !item.isActive,
            });
        } catch (error) {
    ...
    const activeTechnicalDetailCount = (initialData.technicalDetails ?? []).filter((x) => x.isActive).length;
        ...
        <div className="space-y-6">
            <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <Pill tone="neutral">Chi tiết kỹ thuật active: {activeTechnicalDetailCount}</Pill>
                </div>
            </div>
            ...
            <CollapsibleSectionCard
                title="Technical Detail Catalog"
                icon={<Wrench className="h-5 w-5" />}
                description="Chi tiết kỹ thuật theo khu vực: vỏ, máy, núm, kính, mặt số..."
                badge={<Pill tone="gray">{(initialData.technicalDetails ?? []).length} mục</Pill>}
                defaultOpen={false}
            >
                <CollapsibleEditorCard
                    title={editingTechnicalDetailId ? "Chỉnh sửa chi tiết kỹ thuật" : "Thêm chi tiết kỹ thuật"}
                    subtitle="Dùng khi kỹ thuật chuyển issue từ đã xác nhận sang đang xử lý."
                    editing={!!editingTechnicalDetailId}
                    defaultOpen={false}
                >
                    <div className="grid gap-4 md:grid-cols-4">
                        <Field label="Code">
                            <TextInput
                                value={technicalDetailForm.code}
                                onChange={(e) =>
                                    setTechnicalDetailForm((p) => ({ ...p, code: e.target.value }))
                                }
                                placeholder="CASE_POLISH"
                            />
                        </Field>

                        <Field label="Khu vực">
                            <SelectInput
                                value={technicalDetailForm.area}
                                onChange={(e) =>
                                    setTechnicalDetailForm((p) => ({ ...p, area: e.target.value }))
                                }
                            >
                        ...
                        <Field label="Tên chi tiết">
                            <TextInput
                                value={technicalDetailForm.name}
                                onChange={(e) =>
                                    setTechnicalDetailForm((p) => ({ ...p, name: e.target.value }))
                                }
                        ...
                        <Field label="Thứ tự hiển thị">
                            <TextInput
                                type="number"
                                value={technicalDetailForm.sortOrder}
                                onChange={(e) =>
                                    setTechnicalDetailForm((p) => ({
                                        ...p,
                                        sortOrder: Number(e.target.value || 0),
                                    }))
                                }
                        ...
                        <div className="md:col-span-3">
                            <Field label="Mô tả">
                                <TextArea
                                    value={technicalDetailForm.description}
                                    onChange={(e) =>
                                        setTechnicalDetailForm((p) => ({
                                            ...p,
                                    ...
                                    placeholder="Ghi chú ngắn để kỹ thuật hiểu khi nào dùng chi tiết này..."
                                />
                            </Field>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex h-11 items-center rounded-xl border border-slate-200 bg-white px-3 text-sm">
                                <label className="flex items-center gap-2">
                                    <input
                                        type="checkbox"
                                        checked={technicalDetailForm.isActive}
                                        onChange={(e) =>
                                            setTechnicalDetailForm((p) => ({
                                                ...p,
                                                isActive: e.target.checked,
                    ...
                    <div className="mt-4 flex items-center gap-3">
                        <Button
                            onClick={handleSaveTechnicalDetail}
                            disabled={submitting === "technicalDetail"}
                        >
                            {submitting === "technicalDetail"
                                ? "Đang lưu..."
                                : editingTechnicalDetailId
                                    ? "Cập nhật chi tiết"
                                    : "Lưu chi tiết"}
                        </Button>
                ...
                <div className="space-y-3">
                    {(initialData.technicalDetails ?? []).map((item) => (
                        <div
                            key={item.id}
                            className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"
                        >
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button variant="outline" onClick={() => startEditTechnicalDetail(item)}>
                                        <Pencil className="mr-2 h-4 w-4" />
                                        Sửa
                                    </Button>
                                    ...
                                    <Button
                                        variant="outline"
                                        onClick={() => toggleTechnicalDetailActive(item)}
                                    >
                                        {item.isActive ? (
                                            <>
                                                <EyeOff className="mr-2 h-4 w-4" />
