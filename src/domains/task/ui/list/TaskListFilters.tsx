...
import { TaskPriority, TaskStatus, type TaskKind } from "@prisma/client";
import { TASK_PRIORITY_LABEL, TASK_STATUS_LABEL } from "../../utils/task-labels";
...
export default function TaskListFilters({
  ...
  return (
    <div className="grid gap-3 md:grid-cols-[minmax(0,1.4fr)_170px_150px_120px]">
      <input
        value={filters.q}
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={onApply}
          className="rounded-2xl bg-slate-950 px-4 text-sm font-semibold text-white"
        >
          Lọc
        </button>
        <button
          type="button"
          onClick={onClear}
          className="rounded-2xl border border-slate-200 px-4 text-sm font-semibold text-slate-600"
        >
          Xóa
        </button>
      </div>
