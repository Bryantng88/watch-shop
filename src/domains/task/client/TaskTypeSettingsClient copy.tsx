    ...
    <div className="mx-auto w-full max-w-[1360px] min-w-0 space-y-5 px-4 py-6 lg:px-5 xl:px-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold text-slate-950">Task Types & Rules</h1>
          <p className="mt-1 max-w-3xl text-sm text-slate-500">
            Quản lý loại task và rule hoàn tất tự động. Business rule chỉ áp dụng cho system task; manual task vẫn do người xử lý xác nhận.
          </p>
        </div>
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm code, tên, mô tả, rule..." className="h-10 rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:border-slate-400" />
      ...
      <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
        <div className="grid grid-cols-[160px_1.2fr_130px_220px_120px_90px] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">
          <div>Code</div>
          <div>Tên</div>
          <div>Domain</div>
          <div>Completion</div>
        ...
        {filtered.length ? filtered.map((item) => (
          <button key={item.id} type="button" onClick={() => openEdit(item)} className="grid w-full grid-cols-[160px_1.2fr_130px_220px_120px_90px] gap-3 border-b border-slate-100 px-4 py-3 text-left text-sm transition hover:bg-slate-50 last:border-b-0">
            ...
            <div>
              ...
              {item.completionRuleKey ? <div className="mt-0.5 font-mono text-[11px] text-slate-400">{getTaskCompletionRuleLabel(item.completionRuleKey)}</div> : null}
            </div>
