...
function normalizeInitial(initial: any) {
    ...
    const complicationIds =
        ws?.complication?.map((c: any) => c.id) ?? initial?.complications?.map((c: any) => c.id) ?? [];
    ...
    return {
        ...
        variantAvailabilityStatus: firstVariant?.availabilityStatus ?? 'ACTIVE',
        ...
        variantCostPrice:
            firstVariant?.costPrice != null
                ...
                : firstVariant?.acquisitionItem?.[0]?.unitCost != null
                    ? Number(firstVariant.acquisitionItem[0].unitCost)
                    : '',
        ...
        try {
            ...
            const variantPart = sanitizeDeep(variantRaw)
                ? {
                    ...
                    costPrice: variantRaw.variantCostPrice,
                }
                : undefined;
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
                <div className="space-y-6 xl:col-span-8">
                    <div className="rounded-md border border-gray-200 bg-white p-5 shadow space-y-5">
                        <SectionTitle title="Chỉnh sửa sản phẩm" subtitle="Bổ sung đầy đủ thông tin cơ bản, variant và watch spec để sản phẩm sẵn sàng đăng." />

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <InputField label="Tên sản phẩm" name="title" value={formData.title} onChange={handleChange as any} />
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <SelectField label="Loại sản phẩm" name="type" value={formData.type} onChange={handleChange as any} options={typeOptions} placeholder="-- Chọn loại --" />
                            <SelectField label="Trạng thái" name="status" value={formData.status} onChange={handleChange as any} options={productStatusOptions} placeholder="-- Chọn trạng thái --" />
                            <InputField label="SEO title" name="seoTitle" value={formData.seoTitle} onChange={handleChange as any} />
                        </div>
                    ...
                    <div className="rounded-md border border-gray-200 bg-white p-5 shadow space-y-5">
                        <SectionTitle title="Product variant" subtitle="Các trường này đang được dùng để kiểm tra đủ điều kiện đăng." />

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <InputField label="SKU" name="variantSku" value={formData.variantSku} onChange={handleChange as any} />
                            <InputField label="Giá bán" name="variantPrice" value={formData.variantPrice} onChange={handleChange as any} type="number" />
                            <InputField label="Giá nhập" name="variantCostPrice" value={formData.variantCostPrice} onChange={handleChange as any} type="number" />
                        </div>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <InputField label="Giá sale" name="variantSalePrice" value={formData.variantSalePrice} onChange={handleChange as any} type="number" />
                            <InputField label="Tồn kho" name="variantStockQty" value={formData.variantStockQty} onChange={handleChange as any} type="number" />
                            <SelectField
                    ...
                    <div className="rounded-md border border-gray-200 bg-white p-5 shadow space-y-5">
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            <InputField label="Model" name="model" value={formData.model} onChange={handleChange as any} />
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            <SelectField label="Chất liệu vỏ" name="caseMaterial" value={formData.caseMaterial} onChange={handleChange as any} options={caseMaterialOptions} placeholder="-- Chọn chất liệu --" />
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            <InputField label="Case size" name="caseSize" value={formData.caseSize} onChange={handleChange as any} placeholder="Ví dụ 39mm" />
                        </div>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <InputField label="Dài" name="length" value={formData.length} onChange={handleChange as any} type="number" />
                            <InputField label="Rộng" name="width" value={formData.width} onChange={handleChange as any} type="number" />
                            <InputField label="Độ dày" name="thickness" value={formData.thickness} onChange={handleChange as any} type="number" />
                        </div>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <InputField label="Màu mặt số" name="dialColor" value={formData.dialColor} onChange={handleChange as any} />
                            <SelectField label="Dây / Strap" name="strap" value={formData.strap} onChange={handleChange as any} options={strapOptions} placeholder="-- Chọn strap --" />
                            <SelectField label="Kính" name="glass" value={formData.glass} onChange={handleChange as any} options={glassOptions} placeholder="-- Chọn kính --" />
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                            <label className="flex items-center gap-2 rounded border px-3 py-2 text-sm">
                                <input type="checkbox" name="boxIncluded" checked={!!formData.boxIncluded} onChange={handleChange as any} />
                                Kèm hộp
                            </label>
                            <label className="flex items-center gap-2 rounded border px-3 py-2 text-sm">
                                <input type="checkbox" name="bookletIncluded" checked={!!formData.bookletIncluded} onChange={handleChange as any} />
                                Kèm sổ
                            </label>
                            <label className="flex items-center gap-2 rounded border px-3 py-2 text-sm">
                                <input type="checkbox" name="cardIncluded" checked={!!formData.cardIncluded} onChange={handleChange as any} />
                                Kèm thẻ
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="rounded-md border border-gray-200 bg-white p-5 shadow">
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                            <div>
                                <SectionTitle title="Hình ảnh" subtitle="Ảnh đầu tiên sẽ được dùng làm ảnh đại diện nếu bạn chưa chọn riêng." />
                                <div className="mt-4">
                ...
                <aside className="space-y-6 xl:col-span-4 xl:sticky xl:top-6">
                    <div className="rounded-md border border-gray-200 bg-white p-5 shadow space-y-4">
                        <SectionTitle title="Phiếu nhập hàng" subtitle="Khối này giữ thông tin nhập cơ bản để sửa nhanh ngay tại form product." />

                        <InputField label="Giá nhập" name="variantCostPrice" value={formData.variantCostPrice} onChange={handleChange as any} type="number" />
                        <InputField label="Tiền tệ" name="currency" value={formData.currency} onChange={handleChange as any} />

                        <div className="rounded-lg border bg-gray-50 p-3 text-sm text-gray-600">
                            Nếu cần sửa sâu phiếu nhập hoặc nhiều item, nên chỉnh ở module nhập hàng. Ở đây ưu tiên thao tác nhanh cho 1 sản phẩm.
                        </div>
                    </div>
                </aside>
            </div>
