...
export async function createOrderWithItems(raw: any) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const updatedOrder = await updateOrderSubtotalRepo(tx as any, order.id, subtotal);
    // Tạo đơn chỉ lưu DRAFT. Inventory sẽ sync khi confirm/post order.

    return toPlain({
      id: order.id,
      refNo: updatedOrder.refNo ?? refNo,
      status: updatedOrder.status ?? input.status ?? OrderStatus.DRAFT,
      subtotal: toNumberPrice(updatedOrder.subtotal),
    });
  });
}
